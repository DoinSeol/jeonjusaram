...
<div class="wrap">
  ...
  <div class="data_tables">
    ...
    <table summary="${menu_title} 목록">
      ...
      if(record_count > 0)
      {
        ...
        while(rs_list.next())
        {
          ...
          //rs_bsubject = "<a href='program.jbe?" + req_unify_str + "&amp;method=view&amp;idx=" + rs_seqid + "&amp;page=" + rq_page + "' alt='" + rs_bsubject + "'>" + rs_bsubject + "</a>";
          
          if(rs_unify_cnt == 0)
          {
            rs_bsubject = "<a href='board.jbe?cmsid=" + rs_cid + "&amp;idx=" + rs_seqid + "&amp;method=v' title='" + rs_bsubject + "'>" + rs_bsubject + "</a>";
          }
          else
          {
            rs_bsubject = "<a href='board_unify.jbe?cmsid=" + rs_unify_cid + "&amp;idx=" + rs_seqid + "&amp;method=v' title='" + rs_bsubject + "'>" + rs_bsubject + "</a>";
          }
    ...
    <div id="pagingNav">
      ...
      {
        // 첫페이지로
        out.println("<a class='first' href='program.jbe?" + req_unify_str + "&amp;method=list&amp;idx=&amp;page=1'>처음</a>");
        
        // 이전10개페이지보여주기
        out.println("<a class='previ' href='program.jbe?" + req_unify_str + "&amp;method=list&amp;idx=&amp;page=" + (intStart - intNumOfPage) + "'>이전</a>");
      }
      else
      {
      ...
      for(i = intStart; i <= intEnd; i++)
      {
        ...
        {
           out.println("<a class='num' href='program.jbe?" + req_unify_str + "&amp;method=list&amp;idx=&amp;page=" + i + "'>" + i + "</a>");
        }
      }
      ...
      {
        // 다음10개페이지로
        out.println("<a class='next' href='program.jbe?" + req_unify_str + "&amp;method=list&amp;idx=&amp;page=" + (intEnd + 1) + "'>다음</a>");
  
        // 마지막페이지로
        out.println("<a class='last' href='program.jbe?" + req_unify_str + "&amp;method=list&amp;idx=&amp;page=" + totPage + "'>마지막</a>");
      }
      else
      {
